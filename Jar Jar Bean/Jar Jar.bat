...
FOR %%L IN (4 5) DO IF %MASK%==%%L (
	SET FILENAME=%6
	CALL :Merge %5 %FILENAME%
	IF %MASK%==4 SET INPUT_XML=%TEMP%
	IF %MASK%==5 SET OUTPUT_XML=%TEMP%
)
ECHO GO
CALL :CHECK_XML_PATHS
FOR %%M IN (1 3 4) DO IF %MASK%==%%M ( CALL :Input ) ELSE ( GOTO:EOF )
FOR %%N IN (2 3 5) DO IF %MASK%==%%N ( CALL :Output ) ELSE ( GOTO:EOF )
GOTO:EOF
...
:Copy
SET /A Compteur+=1
ECHO F | CALL "%~dp0/../Tools/xcopy.exe" /C /I /Q %1 "%TEMP%temp%Compteur%_%2" > nul
GOTO:EOF
