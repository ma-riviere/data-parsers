...
REM IF "%JAR%"=="3" GOTO RIDES
REM IF "%JAR%"=="4" GOTO COOLTIMES
REM IF "%JAR%"=="5" GOTO ITEMS
REM IF "%JAR%"=="6" GOTO RECIPES
REM IF "%JAR%"=="7" GOTO SKILLS
REM IF "%JAR%"=="8" GOTO SKILL_LEARN
REM IF "%JAR%"=="9" GOTO NPCS
IF %JAR%==10 CALL :MAIN animations 1 800 1024 %ANIMATIONS%
REM IF "%JAR%"=="11" GOTO HOUSING
IF %JAR%==12 CALL :MAIN mission 6 800 1024 %LEVELS% *mission0.xml %SPAWNS% *.xml
REM IF "%JAR%"=="13" GOTO WORLD_DATA
