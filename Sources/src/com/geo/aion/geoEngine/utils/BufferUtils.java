package com.geo.aion.geoEngine.utils;
...
import com.geo.aion.geoEngine.math.Vector2f;
import com.geo.aion.geoEngine.math.Vector3f;

/**
 * <code>BufferUtils</code> is a helper class for generating nio buffers from jME data classes such as Vectors and
 * ColorRGBA.
 * 
 * @author Joshua Slack
...
public final class BufferUtils {
	...
	/**
	 * Generate a new FloatBuffer using the given array of Vector3f objects. The FloatBuffer will be 3 * data.length long
	...
	/**
	 * Create a new FloatBuffer of an appropriate size to hold the specified number of Vector3f object data only if the
	 ...
	 */
	/*
	 * public static void setInBuffer(ColorRGBA color, FloatBuffer buf, int index) { buf.position(index*4);
	 * buf.put(color.r); buf.put(color.g); buf.put(color.b); buf.put(color.a); }
	 */
	...
	/**
	 * Copies a Vector3f from one position in the buffer to another. The index values are in terms of vector number (eg,
	 * vector number 0 is postions 0-2 in the FloatBuffer.)
	 * 
	 * @param buf
	...
	/**
	 * Generate a new FloatBuffer using the given array of Vector2f objects. The FloatBuffer will be 2 * data.length long
	...
	/**
	 * Create a new FloatBuffer of an appropriate size to hold the specified number of Vector2f object data only if the
	 * given buffer if not already the right size.
	 * 
	 * @param buf
	 *          the buffer to first check and rewind
	...
	/**
	 * Generate a new IntBuffer using the given array of ints. The IntBuffer will be data.length long and contain the int
	 * data as data[0], data[1]... etc.
	 * 
	 * @param data
	...
	/**
	 * Creates a new DoubleBuffer with the same contents as the given DoubleBuffer. The new DoubleBuffer is seperate from
	...
	/**
	 * Creates a new FloatBuffer with the same contents as the given FloatBuffer. The new FloatBuffer is seperate from the
	...
	/**
	 * Create a new IntBuffer of an appropriate size to hold the specified number of ints only if the given buffer if not
	...
	/**
	 * Create a new ByteBuffer of an appropriate size to hold the specified number of ints only if the given buffer if not
	 * already the right size.
	 * 
	 * @param buf
	 *          the buffer to first check and rewind
	...
	/**
	 * Create a new ShortBuffer of an appropriate size to hold the specified number of shorts only if the given buffer if
	...
	/**
	 * Creates a new ShortBuffer with the same contents as the given ShortBuffer. The new ShortBuffer is seperate from the
	 * old one and changes are not reflected across. If you want to reflect changes, consider using Buffer.duplicate().
	 * 
	 * @param buf
	 ...
	 * @return a buffer large enough to receive at least the <code>required</code> number of entries, same position as the
